<?
    ...
    if(isset($_POST['sendText'])){
        ...
        header("location:../Ostate/Ostate.php?id=$id");
    }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <link rel="stylesheet" href="Ostate.css">
...
<body>
    <div class="wrapper">
        ...
        <?
            $thisPost = $conn -> query("SELECT `posts`.*, `users`.*
FROM `posts` 
	LEFT JOIN `users` ON `posts`.`userId` = `users`.`id` WHERE `posts`.`id` = '$id'");
        ...
        <div class="posts-container">
            <div class="text">
                <h1>Пост-информация</h1>
            </div>
            <div class="post">
                <div class="post-header">
                    <img src="2.jpg" alt="logo" class="avatar">
                    <div class="info">
                        <h2><?=$value['login']?></h2>
                ...
                <!-- <div class="rating-section">
                    <form class="rating-stars" method="post">
                        <button class="star" data-value="1" value="1" name="currentRatingValue" style="background-color: rgb(28, 28, 28); border: 0;">★</button>
                        <button class="star" data-value="2" value="2" name="currentRatingValue" style="background-color: rgb(28, 28, 28); border: 0;">★</button>
                        <button class="star" data-value="3" value="3" name="currentRatingValue" style="background-color: rgb(28, 28, 28); border: 0;">★</button>
                        <button class="star" data-value="4" value="4" name="currentRatingValue" style="background-color: rgb(28, 28, 28); border: 0;">★</button>
                        <button class="star" data-value="5" value="5" name="currentRatingValue" style="background-color: rgb(28, 28, 28); border: 0;">★</button>
                    </form>
                    <div class="rating-text">Оцените пост</div>
                    <form method="post" class="rateSubmit">
                    </form>
                </div> -->
                <div class="rating-section">
                    <form class="rating-stars" method="post">
...
FROM `posts` 
	...
	LEFT JOIN `users` ON `comments`.`userId` = `users`.`id` WHERE `comments`.`postId` = '$id'");
   ...
   <script>
        ...
        document.querySelectorAll('.rating-stars').forEach(starsContainer => {
            ...
            <?
                $current = $conn -> query("SELECT `posts`.*, `likes`.*
FROM `posts` 
	LEFT JOIN `likes` ON `likes`.`postId` = `posts`.`id` WHERE `likes`.`postId` = '$id';");
          ...
          <?  }
            ...
            stars.forEach(star => {
                star.addEventListener('click', function() {
                    ...
                    <?
                        $currentRateValue = $_POST['currentRatingValue'];
                        $conn -> query("INSERT INTO `likes`(`currentRate`, `userId`, `postId`) VALUES ('$currentRateValue','{$_SESSION['user']['id']}','$id')");
                    ?>
                });
